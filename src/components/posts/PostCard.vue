...
async function toggleLike() {
  ...
  try {
    if (!likedByMe.value) {
      const { data } = await createLike({ userId: Number(authStore.user?.id), postId: props.post.id, commentId: null })
...
<template>
  <article class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 transition-shadow hover:shadow-md">
    ...
    <p @click="openPost" class="text-sm text-slate-700 dark:text-slate-300 leading-relaxed mb-3 cursor-pointer hover:text-slate-900 dark:hover:text-white">
      {{ post.postContent }}
    </p>

    <div v-if="post.imageUrl" class="mb-3 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800">
      <img :src="post.imageUrl" :alt="post.postContent" @click="openPost" class="w-full max-h-96 object-cover cursor-pointer" loading="lazy" @error="$event.target.style.display = 'none'" />
    </div>

    <div v-if="post.categories?.length" class="flex flex-wrap gap-1.5 mb-3">
      <RouterLink v-for="cat in post.categories" :key="cat.categoryId" :to="{ name: 'feed', query: { categories: cat.categoryId } }" :class="getCategoryColor(cat.categoryId)" class="text-xs font-medium px-2.5 py-0.5 rounded-full hover:opacity-75 transition-opacity cursor-pointer" @click.stop>
        {{ cat.name }}
      </RouterLink>
    </div>

    <div class="flex items-center gap-5 pt-3 border-t border-slate-100 dark:border-slate-800">
      <button @click="toggleLike" :disabled="likeLoading" class="flex items-center gap-1.5 text-sm transition-colors" :class="likedByMe ? 'text-rose-600' : 'text-slate-500 dark:text-slate-400 hover:text-rose-600'">
        <svg xmlns="http://www.w3.org/2000/svg" :fill="likedByMe ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-5 h-5">
