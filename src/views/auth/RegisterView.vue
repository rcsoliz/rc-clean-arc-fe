  ...
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center px-4">
    <div class="w-full max-w-sm">
      <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-semibold text-slate-900 dark:text-white">Crea tu cuenta</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Únete a la comunidad</p>
      </div>
      ...
      <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div>
            ...
            <input v-model="username" type="text" required placeholder="carlos" class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500" />
          </div>
          <div>
          ...
          <div>
            ...
            <input v-model="password" type="password" required minlength="6" placeholder="Mínimo 6 caracteres" class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500" />
          </div>
          ...
          <div v-if="successMessage" class="text-sm text-green-700 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg px-3 py-2">{{ successMessage }}</div>
      ...
      <p class="text-center text-sm text-slate-500 dark:text-slate-400 mt-6">
        ¿Ya tienes cuenta?
        <RouterLink to="/login" class="text-violet-600 font-medium hover:underline">Inicia sesión</RouterLink>
      </p>
    </div>
  </div>
</template>
